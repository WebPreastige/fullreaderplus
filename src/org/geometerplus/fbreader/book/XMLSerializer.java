...
class XMLSerializer extends AbstractSerializer {
	...
	private String serialize(Bookmark bookmark) {
        final StringBuilder buffer = new StringBuilder();
        appendTag(
        ...
        appendTag(
            buffer, "history", true,
            "date-creation", formatDate(bookmark.getDate(DateType.Creation)),
            "date-modification", formatDate(bookmark.getDate(DateType.Modification)),
            "date-access", formatDate(bookmark.getDate(DateType.Access)),
    ...
    private String serialize(Quote quote) {
        final StringBuilder buffer = new StringBuilder();
        appendTag(
            buffer, "quote", false,
            "id", String.valueOf(quote.getId()),
            "visible", String.valueOf(quote.IsVisible)
        );
        appendTag(
            buffer, "book", true,
            "id", String.valueOf(quote.getBookId()),
            "title", quote.getBookTitle()
        );
        appendTagWithContent(buffer, "text", quote.getText());
        appendTag(
            buffer, "history", true,
            "date-creation", formatDate(quote.getDate(DateType.Creation)),
            "date-modification", formatDate(quote.getDate(DateType.Modification)),
            "date-access", formatDate(quote.getDate(DateType.Access)),
            "access-count", String.valueOf(quote.getAccessCount())
        );
        appendTag(
            buffer, "position", true,
            "model", quote.ModelId,
            "paragraph", String.valueOf(quote.getParagraphIndex()),
            "element", String.valueOf(quote.getElementIndex()),
            "char", String.valueOf(quote.getCharIndex())
        );
        closeTag(buffer, "quote");
        return buffer.toString();
    }

    @Override
    public ITextMarker deserializeTextMarker(String xml) {
        try {
    ...
    public Quote deserializeQuote(String xml) {
        try {
            final QuoteDeserializer deserializer = new QuoteDeserializer();
            Xml.parse(xml, deserializer);
            return deserializer.getQuote();
        } catch (SAXException e) {
            System.err.println(xml);
            e.printStackTrace();
            return null;
    ...
    private static final class QuoteDeserializer extends DefaultHandler {
        private static enum State {
            READ_NOTHING,
            READ_BOOKMARK,
            READ_TEXT
        ...
        public Quote getQuote() {
            return myState == State.READ_NOTHING ? myQuote : null;
        }
    ...
    @Override
    public String serialize(ITextMarker textMarker) {
        if(textMarker instanceof Bookmark)
            return serialize((Bookmark)textMarker);
        else 
            if(textMarker instanceof Quote)
                return serialize((Quote)textMarker);
