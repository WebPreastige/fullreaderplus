...
public class BookCollection extends AbstractBookCollection {
	...
	public List<Book> getBooksByTitle(String title){
		List<Book> books = myDatabase.getBooksByTitle(title);
		return books;
	}
	...
	public void removeBook(Book book, boolean deleteFromDisk) {
		synchronized (myBooksByFile) {
			...
			DatabaseHandler handler = new DatabaseHandler(ReaderApplication.getContext());
			FrDocument frDocument = new FrDocument(-1, book.getTitle(), book.File.getPath(), FrDocument.DOCTYPE_FB, FrDocument.getDate());
	...
	public List<Book> booksForLocation(String location){
		final LinkedList<Book> filtered = new LinkedList<Book>();
		for (Book b : books()) {
			if (b.File.getParent().getPath().equals(location)){
				filtered.add(b);
			}
		}
		return filtered;
	...
	public Book getRecentBookByTitleAndLocation(String title, String location){
		List<Long> recentIds = myDatabase.loadRecentBookIds();
		for (long id : recentIds){
			Book book = getBookById(id);
			if (book.getTitle().equals(title) && book.File.getPath().equals(location)){
				return book;
			}
		}
		return null;
	...
	public void addBookToRecentList(Book book) {
		Log.d("MyLog", "Add book to recent");
		final List<Long> ids = myDatabase.loadRecentBookIds();
		...
		FrDocument frDocument = new FrDocument(-1, book.getTitle(), book.File.getPath(), FrDocument.DOCTYPE_FB, FrDocument.getDate());
		long id = db.hasFrDocument(frDocument);
		if (id == -1){
			db.addFrDocument(frDocument);
		}
		else{
			frDocument.updateId((int)id);
			db.updateFrDocumentLastDate(frDocument);
		}
	}
...
//		}
    ...
    public void saveBookmark(ITextMarker quote) {
        if (quote != null) {
            quote.setId(myDatabase.saveBookmark((Bookmark) quote));
        }
    }

    public void deleteBookmark(ITextMarker quote) {
        if (quote != null && quote.getId() != -1) {
            myDatabase.deleteBookmark((Bookmark) quote);
        }
    }	
	/*
	 * 
	 * QOUTES 
	 */
    public List<Quote> quotes(long fromId, int limitCount) {
        return myDatabase.loadVisibleQuotes(fromId, limitCount);
    }

    public List<Quote> quotesForBook(Book book, long fromId, int limitCount) {
        return myDatabase.loadVisibleQuotes(book.getId(), fromId, limitCount);
    }

    public List<Quote> invisibleQuotes(Book book) {
        final List<Quote> list = myDatabase.loadInvisibleQuotes(book.getId());
        Collections.sort(list, new Quote.ByTimeComparator());
        return list;
    }

//    public void saveTextMarker(ITextMarker quote) {
//        if (quote != null) {
//            quote.setId(myDatabase.saveQuote((Quote) quote));
//        }
//    }

    public void deleteBookmarkTextMarker(ITextMarker bookmark) {
        if (bookmark != null && bookmark.getId() != -1) {
            myDatabase.deleteBookmark((Bookmark) bookmark);
        }
    }
    
    public void deleteQuotesTextMarker(ITextMarker quote) {
        if (quote != null && quote.getId() != -1) {
            myDatabase.deleteQuote((Quote) quote);
        }
    }
    /*
     * !QUOTES
    ...
    public void saveBookmark(Bookmark bookmark) {
        if (bookmark != null) {
            bookmark.setId(myDatabase.saveBookmark((Bookmark) bookmark));
        }
    }
