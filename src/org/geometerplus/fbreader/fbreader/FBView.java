...
public final class FBView extends ZLTextView {
	private ReaderApp myReader;

	FBView(ReaderApp reader) {
		super(reader);
		myReader = reader;
	}
	...
	public boolean onFingerSingleTap(int x, int y) {
		...
		// Проверка, на установлена ли опция "только по двойному касанию" для тап зон
		if (!myReader.EnableDoubleTapOption.getValue()){
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextWordRegionSoul) {
				...
				doSelectRegion =
					...
					ReaderApp.ImageTappingAction.doNothing;
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextHyperlinkRegionSoul ||
				...
				if (myReader.WordTappingActionOption.getValue() !=
					ReaderApp.WordTappingAction.doNothing) {
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextWordRegionSoul) {
				doRunAction =
					...
					ReaderApp.WordTappingAction.openDictionary;
			} else if (soul instanceof ZLTextImageRegionSoul) {
				doRunAction =
					myReader.ImageTappingActionOption.getValue() ==
					ReaderApp.ImageTappingAction.openImageView;
			}
	...
	public ZLFile getWallpaperFile() {
		String filePath = myReader.getColorProfile().WallpaperOption.getValue();
		
		if ("".equals(filePath)) {
			//filePath = "wallpapers/battered_paper.png";
			return null;
		}
	...
	private class Footer implements FooterArea {
		...
		public synchronized void paint(ZLPaintContext context) {
			...
			final ReaderApp reader = myReader;
			if (reader == null) {
				return;
			}
