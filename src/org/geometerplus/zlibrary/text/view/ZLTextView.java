...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private ArrayList<ZLTextHighlighting> mHighlightings;
	...
	public synchronized void onScrollingFinished(PageIndex pageIndex) {
		...
		case previous:
		{
			((ReaderApp)ReaderApp.Instance()).refUnclick();
	...
	private void drawSelectionCursorBitmap(ZLPaintContext context, ZLTextSelection.Point pt, Bitmap path, boolean isLeft) {
		if (pt == null) {
			return;
		}
		...
		FullReaderActivity.getInstance().getWindowManager().getDefaultDisplay().getMetrics(metrics);

		Bitmap resizedBitmap;
		switch(metrics.densityDpi){
	     case DisplayMetrics.DENSITY_LOW:
	    	 resizedBitmap = Bitmap.createBitmap(path, 0, 0, (int)(width*0.375), (int)(height*0.375));
	                break;
	     case DisplayMetrics.DENSITY_MEDIUM:
	    	 resizedBitmap = Bitmap.createBitmap(path, 0, 0, (int)(width/2), (int)(height/2));
	                 break;
	     case DisplayMetrics.DENSITY_HIGH:
	    	 resizedBitmap = Bitmap.createBitmap(path, 0, 0, (int)(width*.75), (int)(height*.75));
	                 break;
	     default:
	 		resizedBitmap = Bitmap.createBitmap(path, 0, 0, width, height);
	    	 break;
	}
	
		final int w = ZLTextSelectionCursor.getWidth() ;
		final int h = ZLTextSelectionCursor.getHeight();
		final int a = ZLTextSelectionCursor.getAccent();

		if(isLeft)
			context.drawImage(pt.X - w,  pt.Y + h, resizedBitmap);
		else
			context.drawImage(pt.X ,  pt.Y + h, resizedBitmap);
	}
	...
	public synchronized void paint(ZLPaintContext context, PageIndex pageIndex) {
		...
		if(wallpaper != null) {
			Log.d("WALLPAPER NOT NULL", wallpaper.getPath());
		} else {
			Log.d("WALLPAPER NULL", "WALLPAPER NULL");
		}
		if (wallpaper != null) {
		...
		try {
		drawSelectionCursorBitmap(context, getSelectionCursorPoint(page, ZLTextSelectionCursor.Left), 
				BitmapFactory.decodeStream(ZLFile.createFileByPath("left_point.png").getInputStream()), true);
		drawSelectionCursorBitmap(context, getSelectionCursorPoint(page, ZLTextSelectionCursor.Right), 
				BitmapFactory.decodeStream(ZLFile.createFileByPath("right_point.png").getInputStream()), false);
		} catch (IOException e) {
			e.printStackTrace();
		}
...
//		drawSelectionCursor(context, getSelectionCursorPoint(page, ZLTextSelectionCursor.Right));
	...
	private void drawTextLine(ZLTextPage page, ZLTextLineInfo info, int from, int to, int y) {
		...
		// В случае, если для книги есть выделенные цветом цитаты - их нужно отрисовать
		if(mInfoList!=null){
			int size = mInfoList.size();
			if (size>0){
				ZLTextHighlighting highlighting;
				SelectedMarkInfo sInfo;
				for (int i=0; i<size; i++){
					highlighting = mHighlightings.get(i);
					sInfo = mInfoList.get(i);
					drawBackgroung(highlighting, sInfo.color, page, info, from, to, y);
				}
			}
		}
	...
	public ZLTextFixedPosition getFirstWordStart() {



		return  new ZLTextFixedPosition(
				myCurrentPage.StartCursor.getParagraphIndex(),
				0,	0);
	}

	public ZLTextFixedPosition getFirstWordEnd() {
		return  new ZLTextFixedPosition(
				myCurrentPage.StartCursor.getParagraphIndex(),
				0,	0);
	}
	...
	// ------- Метод, который добавляет в выделенную цветом цитату в список таких цитат для отрисовки
	public void addColorMark(SelectedMarkInfo info){
		if (mInfoList == null){
			mInfoList = new ArrayList<SelectedMarkInfo>();
			mHighlightings = new ArrayList<ZLTextHighlighting>();
		}
		mInfoList.add(info);
		ZLTextHighlighting highlighting = new ZLTextHighlighting();
		highlighting.setup(info.startCursor, info.endCursor);
		mHighlightings.add(highlighting);
	}
	
	// ------ Метод, который получает готовый список с подсвеченными цитатами ------
	public void setColorMarkList(ArrayList<SelectedMarkInfo> infoList){
		mInfoList = infoList;
		mHighlightings = new ArrayList<ZLTextHighlighting>();
		for (SelectedMarkInfo info : infoList){
			ZLTextHighlighting highlighting = new ZLTextHighlighting();
			highlighting.setup(info.startCursor, info.endCursor);
			mHighlightings.add(highlighting);
		}
	...
	// ------- Метод, который удаляет выделнную цветом цитату из списка -------
	public void deleteColorMark(long qId){
		long deleteNum = -1;
		int size = mInfoList.size();
		...
		if (deleteNum !=-1){
			mInfoList.remove(deleteNum);
			ZLTextHighlighting highlighting = mHighlightings.get((int)deleteNum);
			highlighting.clear();
			mHighlightings.remove(deleteNum);
			Application.getViewWidget().reset();
			Application.getViewWidget().repaint();
		}
	}
