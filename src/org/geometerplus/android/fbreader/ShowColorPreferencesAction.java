...
class ShowColorPreferencesAction extends FBAndroidAction {
	ShowColorPreferencesAction(FullReaderActivity baseActivity, ReaderApp fbreader) {
	...
	protected void run(Object ... params) {
		...
		final Intent intent =
				new Intent(BaseActivity.getApplicationContext(), PreferenceActivity.class).putExtra("fromBook", true);
		if (params.length == 1 && params[0] instanceof String) {
			intent.putExtra(PreferenceActivity.SCREEN_KEY, (String)params[0]);
		}
		intent.putExtra(PreferenceActivity.OPEN_COLOR_PREFERENCES, "colors");
		OrientationUtil.startActivityForResult(BaseActivity, intent, FullReaderActivity.REQUEST_PREFERENCES);
	}
}
