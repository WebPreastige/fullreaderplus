...
public final class SQLiteBooksDatabase extends BooksDatabase {
	...
	private SQLiteBooksDatabase(Context context) {
		myDatabase = context.openOrCreateDatabase( "books.sqlite", Context.MODE_PRIVATE, null);
		migrate();
	}
	...
	public List<Book> getBooksByTitle(String title){
		...
		Book book = null;
		final Cursor cursor = myDatabase.rawQuery("SELECT book_id, file_id,title,encoding,language FROM Books WHERE title = '" + title + "'", null);
		if (cursor.moveToNext()) {
			do{
				book = createBook(
						cursor.getLong(0), cursor.getLong(1), cursor.getString(2), cursor.getString(3), cursor.getString(4)
				);
				books.add(book);
			}
	...
	public void updateBookLocation(long bookId, String encoding, String language, String location){
	...
	protected long saveBookmark(Bookmark bookmark) {
		...
		SQLiteUtil.bindDate(statement, 3, bookmark.getDate(DateType.Creation));
		SQLiteUtil.bindDate(statement, 4, bookmark.getDate(DateType.Modification));
		SQLiteUtil.bindDate(statement, 5, bookmark.getDate(DateType.Access));
    ...
    @Override
    protected List<Quote> loadInvisibleQuotes(long bookId) {
        LinkedList<Quote> list = new LinkedList<Quote>();
        Cursor cursor = myDatabase.rawQuery(
            "SELECT qt.quote_id,qt.book_id,b.title,qt.quote_text,qt.creation_time,qt.modification_time,qt.access_time,qt.access_counter,qt.model_id,qt.paragraph,qt.word,qt.char FROM Quotes AS qt INNER JOIN Books AS b ON b.book_id = qt.book_id WHERE qt.book_id = ? AND qt.visible = 0", new String[] { String.valueOf(bookId) }
        );
        while (cursor.moveToNext()) {
    ...
    @Override
    protected List<Quote> loadVisibleQuotes(long fromId, int limitCount) {
        LinkedList<Quote> list = new LinkedList<Quote>();
        Cursor cursor = myDatabase.rawQuery("SELECT" +
            " qt.quote_id,qt.book_id,b.title,qt.quote_text," +
            "qt.creation_time,qt.modification_time,qt.access_time,qt.access_counter," +
            "qt.model_id,qt.paragraph,qt.word,qt.char" +
            " FROM Quotes AS qt INNER JOIN Books AS b ON b.book_id = qt.book_id" +
            " WHERE qt.quote_id >= ? AND qt.visible = 1 ORDER BY qt.quote_id LIMIT ?",
            new String[] { String.valueOf(fromId), String.valueOf(limitCount) }
        );
        while (cursor.moveToNext()) {
            list.add(createQuote(
    ...
    @Override
    protected List<Quote> loadVisibleQuotes(long bookId, long fromId, int limitCount) {
        LinkedList<Quote> list = new LinkedList<Quote>();
        Cursor cursor = myDatabase.rawQuery("SELECT" +
            " qt.quote_id,qt.book_id,b.title,qt.quote_text," +
            "qt.creation_time,qt.modification_time,qt.access_time,qt.access_counter," +
            "qt.model_id,qt.paragraph,qt.word,qt.char" +
            " FROM Quotes AS qt INNER JOIN Books AS b ON b.book_id = qt.book_id" +
            " WHERE b.book_id = ? AND qt.quote_id >= ? AND qt.visible = 1" +
            " ORDER BY qt.quote_id LIMIT ?",
            new String[] { String.valueOf(bookId), String.valueOf(fromId), String.valueOf(limitCount) }
        );
        while (cursor.moveToNext()) {
            list.add(createQuote(
    ...
    private SQLiteStatement myInsertQuoteStatement;
    private SQLiteStatement myUpdateQuoteStatement;
    @Override
    protected long saveQuote(Quote quote) {
        SQLiteStatement statement;
        if (quote.getId() == -1) {
            if (myInsertQuoteStatement == null) {
                myInsertQuoteStatement = myDatabase.compileStatement(
                    "INSERT OR IGNORE INTO Quotes (book_id,quote_text,creation_time,modification_time,access_time,access_counter,model_id,paragraph,word,char,visible) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
                );
            }
            statement = myInsertQuoteStatement;
        } else {
            if (myUpdateQuoteStatement == null) {
                myUpdateQuoteStatement = myDatabase.compileStatement(
                    "UPDATE Quotes SET book_id = ?, quote_text = ?, creation_time =?, modification_time = ?,access_time = ?, access_counter = ?, model_id = ?, paragraph = ?, word = ?, char = ?, visible = ? WHERE quote_id = ?"
                );
            }
            statement = myUpdateQuoteStatement;
        }

        statement.bindLong(1, quote.getBookId());
        statement.bindString(2, quote.getText());
        SQLiteUtil.bindDate(statement, 3, quote.getDate(DateType.Creation));
        SQLiteUtil.bindDate(statement, 4, quote.getDate(DateType.Modification));
        SQLiteUtil.bindDate(statement, 5, quote.getDate(DateType.Access));
        statement.bindLong(6, quote.getAccessCount());
        SQLiteUtil.bindString(statement, 7, quote.ModelId);
        statement.bindLong(8, quote.ParagraphIndex);
        statement.bindLong(9, quote.ElementIndex);
        statement.bindLong(10, quote.CharIndex);
        statement.bindLong(11, quote.IsVisible ? 1 : 0);

        if (statement == myInsertQuoteStatement) {
            return statement.executeInsert();
        } else {
            final long id = quote.getId();
            statement.bindLong(12, id);
            statement.execute();
            return id;
        }
    ...
    private SQLiteStatement myDeleteQuoteStatement;
    @Override
    protected void deleteQuote(Quote quote) {
        if (myDeleteQuoteStatement == null) {
            myDeleteQuoteStatement = myDatabase.compileStatement(
                "DELETE FROM Quotes WHERE quote_id = ?"
            );
        }
        myDeleteQuoteStatement.bindLong(1, quote.getId());
        myDeleteQuoteStatement.execute();
    }
/*
 * 
 * !QUOTE 
