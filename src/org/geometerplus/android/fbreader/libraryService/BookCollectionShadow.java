...
public class BookCollectionShadow extends AbstractBookCollection implements ServiceConnection {
	...
	public synchronized List<Book> booksForLocation(String location) {
		if (myInterface == null) {
			return Collections.emptyList();
		}
		try {
			return SerializerUtil.deserializeBookList(
				myInterface.booksForLocation(location)
			);
		} catch (RemoteException e) {
			return Collections.emptyList();
		}
	...
	public List<Book> getBooksByTitle(String title){
		...
		try {
			return SerializerUtil.deserializeBookList(myInterface.getBooksByTitle(title));
		} catch (RemoteException e) {
			return Collections.emptyList();
		}
	}
	
	public synchronized void removeRecentBook(Book bookForRemove) {
		try {
			myInterface.recentBooks().remove((Book)bookForRemove);
		} catch (RemoteException e) {
	...
	public synchronized Book getRecentBookByTitleAndLocation(String title, String location){
		if (myInterface == null){
			return null;
		}
		try{
			return SerializerUtil.deserializeBook(myInterface.getRecentBookByTitleAndLocation(title, location));
		}
		catch (RemoteException e){
			return null;
		}
	...
	public synchronized Book getBookById(long id) {
		...
		try {
			//Log.d("INTERFACE NULL", "INTERFACE NULL");
	 ...
	 * QUOTES
	 */
    public synchronized List<Quote> invisibleQuotes(Book book) {
        if (myInterface == null) {
            return Collections.emptyList();
        }
        try {
            return SerializerUtil.deserializeQuoteList(
                myInterface.invisibleBookmarks(SerializerUtil.serialize(book))
            );
        } catch (RemoteException e) {
            return Collections.emptyList();
    ...
    public synchronized List<Quote> quotes(long fromId, int limitCount) {
        if (myInterface == null) {
            return Collections.emptyList();
        }
        try {
            return SerializerUtil.deserializeQuoteList(myInterface.quotes(fromId, limitCount));
        } catch (RemoteException e) {
            return Collections.emptyList();
        }
    }

    public synchronized List<Quote> quotesForBook(Book book, long fromId, int limitCount) {
        if (myInterface == null) {
            return Collections.emptyList();
        }
        try {
            return SerializerUtil.deserializeQuoteList(myInterface.quotesForBook(
                SerializerUtil.serialize(book), fromId, limitCount
            ));
        } catch (RemoteException e) {
            return Collections.emptyList();
    ...
    public synchronized void saveBookmark(Bookmark quote) {
        if (myInterface != null) {
            try {
                quote.update(SerializerUtil.deserializeBookmark(
                    myInterface.saveBookmark(SerializerUtil.serialize(quote))
                ));
            } catch (RemoteException e) {
            }
        }
    ...
    public synchronized void saveQuote(Quote quote) {
        if (myInterface != null) {
            try {
                quote.update(SerializerUtil.deserializeQuote(
                    myInterface.saveQuote(SerializerUtil.serialize(quote))
                ));
            } catch (RemoteException e) {
            }
        }
    ...
    public synchronized void deleteTextMarker(ITextMarker quote) {
        if (myInterface != null) {
            try {
            		myInterface.deleteQuote(SerializerUtil.serialize(quote));
            		myInterface.deleteBookmark(SerializerUtil.serialize(quote));
            } catch (RemoteException e) {
            }
        }
    }
