...
class ZLBooleanPreference extends ZLCheckBoxPreference {
	...
	private boolean dropboxBooleanPreference = false;
	...
	protected void onClick() {
		...
		if (isChecked() && dropboxBooleanPreference){
			DropboxHelper helper = DropboxHelper.Instance(ReaderApplication.getContext());
			if (!helper.hasLinkedAccount()){
				helper.startLink(mActivity);
			}
		}
	}
	
	public void setupForDropbox(Activity activity){
		mActivity = activity;
		dropboxBooleanPreference = true;
	}
}
