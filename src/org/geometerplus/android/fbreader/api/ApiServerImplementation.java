...
public class ApiServerImplementation extends ApiInterface.Stub implements Api, ApiMethods {
	...
	private volatile ReaderApp myReader;
	private synchronized ReaderApp getReader() {
		if (myReader == null) {
			myReader = (ReaderApp)ReaderApp.Instance();
		}
		return myReader;
	}
		...
		try {
			switch (method) {
				case GET_FBREADER_VERSION:
					return ApiObject.envelope(getReaderVersion());
